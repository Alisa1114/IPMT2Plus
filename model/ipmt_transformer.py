...
class FFN(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(num_fcs - 1):
            layers.append(
                nn.Sequential(
                    nn.Linear(in_channels, feedforward_channels, bias=False),
                    self.activate,
                    nn.Dropout(dropout),
                )
            )
...
class IPMTransformer(nn.Module):
    def __init__(
        ...
        super(IPMTransformer, self).__init__()
        self.embed_dims = embed_dims
        self.num_heads = num_heads
        ...
        for c_id in range(self.num_layers):
            self.cross_layers.append(
                MyCrossAttention(
                    embed_dims, attn_drop=self.dropout, proj_drop=self.dropout
                ),
            )
        ...
        for l_id in range(self.num_layers - 1):
            self.update_q.append(
                nn.Sequential(
                    nn.Conv2d(
                        embed_dims * 2, embed_dims, kernel_size=1, padding=0, bias=False
                    ),
            ...
            self.res_q.append(
                nn.Sequential(
                    nn.Conv2d(
                        embed_dims, embed_dims, kernel_size=3, padding=1, bias=False
                    ),
                    nn.ReLU(inplace=True),
                    nn.Conv2d(
                        embed_dims, embed_dims, kernel_size=3, padding=1, bias=False
    ...
    def get_qry_flatten_input(self, x, qry_masks):
        ...
        for lvl in range(self.num_levels):
            ...
            if qry_masks is not None:
                ...
                qry_mask = F.interpolate(
                    qry_mask.unsqueeze(1), size=(h, w), mode="nearest"
                ).squeeze(1)
        ...
        qry_valid_masks_flatten = torch.cat(
            qry_valid_masks_flatten, dim=1
        )  # [bs, num_elem]
    ...
    def get_supp_flatten_input(self, s_x, supp_mask):
        ...
        supp_mask = F.interpolate(
            supp_mask, size=s_x.shape[-2:], mode="nearest"
        ).squeeze(
            1
        )  # [bs*shot, h, w]
        ...
        for st_id in range(s_x.size(1)):
            ...
            for img_id in range(s_x.size(0)):
                ...
                if obj_mask.sum() == 0:  # To avoid NaN
                    obj_mask[
                        obj_mask.size(0) // 2 - 1 : obj_mask.size(0) // 2 + 1,
                        obj_mask.size(1) // 2 - 1 : obj_mask.size(1) // 2 + 1,
                    ] = True
    ...
    def forward(self, x, qry_masks, s_x, supp_mask, init_mask):
        ...
        prototype_list = []
        for c_id in range(self.num_layers):
            cross_out = self.cross_layers[c_id](
                prototype, k, v, supp_valid_mask, supp_mask_flatten
            )
        ...
        for l_id in range(self.num_layers):
            ...
            qry_outputs_mask, qry_attn_mask = self.forward_prediction_heads(
                prototype, q
            )
            ...
            if l_id < self.num_layers - 1:
                ...
                tmp_q = rearrange(
                    torch.cat((q, tmp_prototype), dim=-1),
                    "b (h w) c -> b c h w",
                    h=60,
                    w=60,
                )
        ...
        out = rearrange(q, "b (h w) c -> b c h w", h=60, w=60)
